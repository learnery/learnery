...
namespace :theme do
  desc "initialize themes"
  task :init => "git:submodules:init" do
    puts "done."
  end
  ...
  task :install => "git:submodules:init" do
    ...
    system "git submodule add --name #{submodule_name} #{origin} #{destination}"
    exit(0)
  end
  
  desc "remove theme (submodule'd or not)"
  task :remove => "git:submodules:init" do
    
    newtext = File.read(".gitmodules").gsub(/\[submodule "theme\-?\d*"\]\n\s*path = theme\n\s*url = .*\n/,'')
    File.open(".gitmodules", "w") do |gitmodule|
      gitmodule.write(newtext)
    end
    puts "Removed declaration from .gitmodules."

    newtext = File.read(".git/config").gsub(/\[submodule "theme\-?\d*"\]\n\s*url = .*\n/,'')
